...
import { Post, PostReply } from "../../shared/interface/Post";
import toast from "react-hot-toast";
import axios from "axios";

// Component for handling actions such as like, reply, repost, and share on a post
const Actions = ({ post }: { post: Post }) => {
  // Fetching current user from Recoil state
  const user = useRecoilValue(userAtom);

  // Checking if the post is liked by the current user
  const likedStatus = post?.likes?.includes(user?._id as string);
  ...
  const handleReply = async () => {
    ...
    setIsReplying(true);
    try {
      const postReplyResponse = await axios.put<PostReply>(
        `/api/posts/reply/` + post._id,
        { text: reply }
      );
      ...
      const updatedPosts = posts.map((p) => {
        if (p._id === post._id) {
          return {
            ...p,
            replies: [...(p.replies || []), postReplyResponse.data],
          };
        }
        return p;
      });
